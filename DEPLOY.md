...
Счётчик уже подключён в `index.html` на адрес `https://megurt.goatcounter.com/count`, и каждая кнопка шлёт событие вида `click-x`, `click-discord`, `click-steam`, `click-coffee`, `click-mail`, `click-copy-mail`. Пока аккаунта нет, запросы просто отскакивают, посетитель этого не видит.

Чтобы данные пошли:

1. https://www.goatcounter.com/signup → в поле **Code** ввести `megurt` (если занято, любой другой код и заменить `megurt.goatcounter.com` в `index.html`), домен сайта `megurte.github.io`, план Personal (бесплатный, для некоммерческих личных сайтов)
2. Подтвердить почту, дашборд откроется на https://megurt.goatcounter.com
3. Первые визиты появятся через минуту после первого захода на страницу
